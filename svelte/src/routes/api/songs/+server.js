...
// Helper to get songs directory
const getSongsDir = () => path.join(process.cwd(), 'songs');
...
export async function GET() {
  try {
    const manifestPath = path.join(getSongsDir(), "manifest.json");
    let manifest = { songs: [] };
    ...
    for (const file of manifest.songs || []) {
      try {
        const songPath = path.join(getSongsDir(), file);
    ...
    return json({ error: "Failed to load songs" }, { status: 500 });
  }
}
...
export async function POST({ request }) {
  try {
    ...
    const { name, composer, description, data } = body;
    ...
    if (sortedEvents.length === 0) {
      return json({ error: "No events to save" }, { status: 400 });
    }
    ...
    // Save MIDI file
    const songPath = path.join(getSongsDir(), fileName);
    fs.writeFileSync(songPath, Buffer.from(midiBytes, 'binary'));
    ...
    return json({ success: true, fileName, song: songData });
  } catch (error) {
    console.error("Error saving song:", error);
    return json({ error: "Failed to save song" }, { status: 500 });
  }
}
